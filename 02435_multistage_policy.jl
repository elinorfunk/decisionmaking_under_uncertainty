...
Output = Here-and-now decision for stage tau """
...
function discretize_scenarios(scenarios)
    ...
    return rounded_scenarios
end

# Reduce scenarios
function reduce_scenarios(scenarios, max_s)
    reduced_scenarios = []
    if length(scenarios) > max_s
        for _ in 1:max_s
            scenario = rand(scenarios)
            push!(reduced_scenarios, scenario)
        end
...
function make_multistage_here_and_now_decision(number_of_sim_periods, tau, current_stock, current_prices)
    ...
    if tau == 1
        ...
        scenarios = generate_scenarios(current_prices, initial_scenarios)
    end
    ...
    for s in 1:length(scenarios_reduced)
        ...
        if tau == 1
            for w in W
                @constraint(model_multistage, sum(y_send[w, q] for q in W if q != w) <= initial_stock[w]) 
            end
        end 
        ...
        if tau == 1
            for w in W
                @constraint(model_multistage, 0 == initial_stock[w] + x[w] +
                            sum(y_rec[w, q] - y_send[w, q] for q in W) - demand_trajectory[w] + m[w] - z[w])
            end
        else
            for w in W
                @constraint(model_multistage, 0 == current_stock[w] + x[w] +
                            sum(y_rec[w, q] - y_send[w, q] for q in W) - demand_trajectory[w] + m[w] - z[w])
            end
        end
        ...
        for w in W
            for q in W
                @constraint(model_multistage, [w in W, q in W], 0 == sum(y_send[w, q]) - sum(y_rec[q, w]))
            end
        end 
