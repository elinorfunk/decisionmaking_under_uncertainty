...
function load_the_data()
    ...
    cost_tr = ones(number_of_warehouses, number_of_warehouses)*10

    #Capacity of warehouse w
    warehouse_capacities = 10*ones(number_of_warehouses)

    #Capacity of the transportation link for each pair of warehouses w1 and w2
    #Call by capacity_tr[w1,w2]
    transport_capacities = 3*ones(number_of_warehouses, number_of_warehouses)
    ...
    return number_of_warehouses, W, cost_miss, cost_tr, warehouse_capacities, transport_capacities, initial_stock, number_of_simulation_periods, sim_T, degradation_factor
end
