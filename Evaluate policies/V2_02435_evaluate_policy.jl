...
# MAx_scenarios for reduction 
scenarios = [5,10,50,100,200,300]
...
for scen in scenarios
    ...
    for e in Expers
        ...
        for tau in sim_T
            ...
            x[(e,tau)], send[(e,tau)], receive[(e,tau)], z[(e,tau)], m[(e,tau)] = make_multistage_here_and_now_decision(number_of_sim_periods, tau, current_stock, current_prices, scen)
    ...
    results[scen] = FINAL_POLICY_COST

end

# Save results as CSV file 
println(results)
results_df = DataFrame(key = collect(keys(results)), value = collect(values(results)))
CSV.write("task2_results.csv", results_df)
